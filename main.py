...
# ================= 配置区域 =================
UE_IP = "127.0.0.1" # 请确保这里是你 UE 电脑的 IP
UE_PORT = 8000
...
class FinalDroneController:
    ...
    def process_frame(self, frame):
        ...
        # 1. 队形变换
        if g_left == "PALM" and g_right == "PALM": self.formation = "TRIANGLE"
        elif g_left == "THREE_ROW_V": self.formation = "LINE_VERTICAL"
        elif g_left == "TWO_ROW_H": self.formation = "LINE_HORIZONTAL"
        ...
        if self.right_hand_active and g_right:
            ...
            # === C. 飞行核心逻辑 (无Roll版) ===
                ...
                if g_right == "FIST":
                    ...
                    # Pitch 控制 (前后)
                    val_pitch = 0.0
                    if p < self.CALIB_PITCH_MIN: val_pitch = (p - self.CALIB_PITCH_MIN) * self.SENS_PITCH
                    elif p > self.CALIB_PITCH_MAX: val_pitch = (p - self.CALIB_PITCH_MAX) * self.SENS_PITCH
                    ...
                    # HUD 文本只显示 P 和 Y
                    msg_action = f"FLY: P{val_pitch:.2f} Y{val_yaw:.2f}"
                    
                    if r_lm:
                        center = (int(r_lm[9].x * w), int(r_lm[9].y * h))
                        self.draw_hud(frame, r, p, y, center)

        if not self.right_hand_active: msg_action = "LOCKED"
        ...
        cv2.putText(frame, msg_action, (20, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 200, 200), 1)
        ...
        if g_left in ["THREE_ROW_V", "TWO_ROW_H"]:
        ...
        osc_msg = [
            ...
            float(self.drone_id),
            float(control_data["roll"]),   # 这里会发送 0.0
